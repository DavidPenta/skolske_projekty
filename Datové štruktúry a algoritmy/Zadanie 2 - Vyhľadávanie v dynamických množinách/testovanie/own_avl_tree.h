...
struct Block* insert1(struct Block* Block, unsigned long long value) {
    int balance, a = 0, b = 0;
    ...
    if (value < Block->value)
        Block->left = insert1(Block->left, value);

    else if (value > Block->value)
        Block->right = insert1(Block->right, value);
...
/*
int search1(struct Block *root, unsigned long long value){
    if(root != NULL){
        if (root->value == value)
            return 1;
        if (root->value > value)
